...
class PSMSegLoader(object):
    def __init__(self, dataset, win_size, step, mode="train"):
        ...
        self.train = data
        self.val = self.test
        self.test_labels = pd.read_csv('datasets/' + dataset + '/test_label.csv').values[:, 1:]
...
class MSLSegLoader(object):
    def __init__(self, dataset, win_size, step, mode="train"):
        ...
        self.win_size = win_size
        self.scaler = StandardScaler()
        data = np.load('datasets/' + dataset + "/MSL_train.npy")
        self.scaler.fit(data)
        data = self.scaler.transform(data)
        test_data = np.load('datasets/' + dataset + "/MSL_test.npy")
        self.test = self.scaler.transform(test_data)
        self.train = data
        self.val = self.test
        self.test_labels = np.load('datasets/' + dataset + "/MSL_test_label.npy")
...
class SMAPSegLoader(object):
    def __init__(self, dataset, win_size, step, mode="train"):
        ...
        self.win_size = win_size
        self.scaler = StandardScaler()
        data = np.load('datasets/' + dataset + "/SMAP_train.npy")
        self.scaler.fit(data)
        data = self.scaler.transform(data)
        test_data = np.load('datasets/' + dataset + "/SMAP_test.npy")
        self.test = self.scaler.transform(test_data)
        self.train = data
        self.val = self.test
        self.test_labels = np.load('datasets/' + dataset + "/SMAP_test_label.npy")
...
class SMDSegLoader(object):
    def __init__(self, dataset, win_size, step, mode="train"):
        ...
        self.win_size = win_size
        self.scaler = StandardScaler()
        data = np.load('/datasets/' + dataset + "/SMD_train.npy")[:,:]
        self.scaler.fit(data)
        data = self.scaler.transform(data)
        test_data = np.load('/datasets/' + dataset + "/SMD_test.npy")[:,:]
        self.test = self.scaler.transform(test_data)
        self.train = data
        data_len = len(self.train)
        self.val = self.train[(int)(data_len * 0.8):]
        self.test_labels = np.load('/datasets/' + dataset + "/SMD_test_label.npy")[:]
...
class SWATSegLoader(Dataset):
    def __init__(self, dataset, win_size, step=1, flag="train"):
        self.flag = flag
        self.step = step
        self.win_size = win_size
        self.scaler = StandardScaler()
        
        train_data = pd.read_csv(os.path.join('/datasets/', dataset, 'swat_train2.csv'))
        test_data = pd.read_csv(os.path.join('/datasets/', dataset, 'swat2.csv'))
...
def get_loader_segment(index, batch_size, win_size=100, step=100, mode='train', dataset='KDD'):
    if (dataset == 'SMD'):
        dataset = SMDSegLoader(dataset, win_size, 1, mode)
    elif (dataset == 'MSL'):
        dataset = MSLSegLoader(dataset, win_size, 1, mode)
    elif (dataset == 'SMAP'):
        dataset = SMAPSegLoader(dataset, win_size, 1, mode)
    elif (dataset == 'PSM'):
        dataset = PSMSegLoader(dataset, win_size, 1, mode)
    elif (dataset =='SWAT'):
        dataset = SWATSegLoader(dataset,win_size,1,mode)
    elif (dataset == 'UCR'):
        dataset = UCRSegLoader(index, dataset, win_size, 1, mode)
    elif (dataset == 'UCR_AUG'):
        dataset = UCRAUGSegLoader(index, dataset, win_size, 1, mode)
    elif (dataset == 'NIPS_TS_Water'):
        dataset = NIPS_TS_WaterSegLoader(dataset, win_size, 1, mode)
    elif (dataset == 'NIPS_TS_Swan'):
        dataset = NIPS_TS_SwanSegLoader(dataset, win_size, 1, mode)
    elif (dataset == 'NIPS_TS_CCard'):
        dataset = NIPS_TS_CCardSegLoader(dataset, win_size, 1, mode)
    elif (dataset == 'SMD_Ori'):
        dataset = SMD_OriSegLoader(index, dataset, win_size, 1, mode)
