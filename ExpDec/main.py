...
if __name__ == '__main__':
    ...
    args.patch_size = [int(patch) for patch in args.patch_size.split('_')]

    if args.dataset == 'UCR':
        batch_size_buffer = [2,4,8,16,32,64,128,256]
        data_len = np.load('datasets/'+args.datasets + "/UCR_"+str(args.index)+"_train.npy").shape[0] 
        args.batch_size = find_nearest(batch_size_buffer, data_len / args.win_size)
    elif args.dataset == 'UCR_AUG':
        batch_size_buffer = [2,4,8,16,32,64,128,256]
        data_len = np.load('datasets/'+args.datasets + "/UCR_AUG_"+str(args.index)+"_train.npy").shape[0] 
        config.batch_size = find_nearest(batch_size_buffer, data_len / args.win_size)
    elif args.dataset == 'SMD_Ori':
        batch_size_buffer = [2,4,8,16,32,64,128,256,512]
        data_len = np.load('datasets/'+args.datasets + "/SMD_Ori_"+str(config.index)+"_train.npy").shape[0] 
        args.batch_size = find_nearest(batch_size_buffer, data_len / args.win_size)
