...
class Exp(Exp_Basic):
    def __init__(self, args):
        ...
        self.mask_generator = MaskGenerator(self.args.input_c, self.args.seq_len) # input dim이 enc_in 맞나??
    ...
    def _build_model(self):
        ...
        self.model = self.model_dict[self.args.model].Model(self.model_config).float().to(self.device) #TODO model_config 만들기
        self.solver = self.solver_dict[self.args.model].Model(self.model_config).float().to(self.device)
    ...
    def _build_diffusion_model(self):
        self.diffusion = Diffusion_TS(seq_length=self.args.win_size, feature_size=self.args.input_c, **kwargs)
