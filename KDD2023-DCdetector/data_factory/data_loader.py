...
class SMDSegLoader(object):
    def __init__(self, data_path, win_size, step, mode="train"):
        ...
        self.win_size = win_size
        self.scaler = StandardScaler()
        data = np.load('../dataset_anomaly/' + data_path + "/SMD_train.npy")[:,:]
        self.scaler.fit(data)
        data = self.scaler.transform(data)
        test_data = np.load('../dataset_anomaly/' + data_path + "/SMD_test.npy")[:,:]
        self.test = self.scaler.transform(test_data)
        self.train = data
        data_len = len(self.train)
        self.val = self.train[(int)(data_len * 0.8):]
        self.test_labels = np.load('../dataset_anomaly/' + data_path + "/SMD_test_label.npy")[:]
...
class SWATSegLoader(Dataset):
    def __init__(self, root_path, win_size, step=1, flag="train"):
        ...
        train_data = pd.read_csv(os.path.join('../dataset_anomaly/', root_path, 'swat_train2.csv'))
        test_data = pd.read_csv(os.path.join('../dataset_anomaly/', root_path, 'swat2.csv'))
